<template>
  <div class="w-1/2 mx-auto my-20">
    <form @submit.prevent="sendForm">
      <label class="flex justify-between gap-2 mb-4 cursor-pointer">
      ...
      <label>
        ...
        <SfInput v-model="usernameModelValue" required :invalid="usernameIsInvalid" @update:model-value="usernameIsInvalid = !usernameModelValue" />
      ...
      <!-- <label class="block my-4">
        ...
        <SfInput v-model="emailModelValue" type="email" :invalid="emailIsInvalid" required @update:model-value="emailIsInvalid = !emailModelValue">
          <template #prefix>
            <SfIconEmail />
          </template>
        </SfInput>
        <p v-if="emailIsInvalid" class="mt-0.5 text-negative-700 typography-text-sm font-medium">The field cannot be empty</p>
      </label>
      <label>
      ...
      <fieldset class="flex flex-wrap my-4">
        ...
        <label class="w-full md:w-[122px] md:mr-4">
          <span class="font-medium typography-label-sm"> Code * </span>
          <SfSelect v-model="areaCode" required placeholder="--" :invalid="areaCodeIsInvalid" @update:model-value="areaCodeIsInvalid = !areaCode">
            <option v-for="({ value, label }, index) in areaCodes" :key="`${value}-${index}`" :value="value">
              {{ label }}
            </option>
          </SfSelect>
          <p v-if="areaCodeIsInvalid" class="mt-0.5 text-negative-700 typography-text-sm font-medium">No option selected</p>
        </label>
        <span class="mt-4 md:mt-0 grow">
          <label>
            <span class="font-medium typography-label-sm"> Phone * </span>
            <SfInput
            ...
            />
            <p v-if="phoneNumberIsInvalid" class="mt-0.5 text-negative-700 typography-text-sm font-medium">The field cannot be empty</p>
          </label>
        </span>
      </fieldset>
      <div ref="referenceRef" class="relative">
        ...
        <div v-if="isOpen" ref="floatingRef" :style="style" class="left-0 right-0 z-10">
          ...
          >
            ...
            <p v-else-if="countryModelValue" class="inline-flex w-full px-4 py-2 text-left" aria-label="No options">
      ...
      <p v-if="!isDisabled && isValid === false" class="text-negative-700 typography-text-sm font-medium mt-0.5">No option selected</p>
      <label class="block mt-4 font-medium typography-label-sm" :for="dropdownID">Your Interests</label>
      <div ref="dropdownReferenceRef" class="relative mb-4">
        ...
        >
          ...
          <SfIconExpandMore class="ml-auto transition-transform duration-300 ease-in-out text-neutral-500" :class="{ 'rotate-180': dropdownOpen }" />
        </div>
        <ul
          v-show="dropdownOpen"
          :id="listboxId"
      ...
      <label>
        ...
        <textarea
          ...
          class="block w-full py-2 pl-4 border rounded-md pr-7 border-neutral-300 placeholder:text-neutral-500"
        ></textarea>
      </label>
      <h2 class="mb-4 font-bold typography-headline-4 mt-14">Notifications</h2>
      <fieldset>
      ...
      <fieldset role="radiogroup">
        ...
        >
          <SfRadio v-model="pushNotificationsModelValue" :invalid="pushNotificationIsInvalid" required :value="value" :name="name" />
          <span class="ml-2 text-base font-normal leading-6 font-body">{{ label }}</span>
        </label>
      </fieldset>
      <p class="mt-8 text-neutral-500 typography-text-sm">* marked fields are required</p>
      <div class="flex mt-6 gap-x-4 md:justify-end">
